...
open class BaseListViewSource: NSObject, ListViewSource {
    ...
    lazy var registrationCells: [CellModel] = { produceCells() }()
    ...
    public func models(forIdentifier identifier: String) -> [ViewModelItem]? {
...
private extension BaseListViewSource {
    ...
    func configure() {
        #if DEBUG
        let loadingBehavior = Observable
            .combineLatest(
    ...
    func updateModel(_ model: ListViewSourceModel) {
        ...
        guard var models = listModels[identifier] else {
            listModels[identifier] = model.data
            return
        }
        switch model.type {
        case .initial, .swap:
    ...
    func toDataSectionWhenModelChanged(from: ([SectionModel], ListModelChangeType)) -> [DataSection] {
        ...
        return sections
            ...
            .map ({
                ...
                return .init(model: enumeration.element.tag, elements: objects(in: enumeration.element, at: enumeration.offset, onChanged: type))
            })
    }
}
